...
function RadarDecodeRadarPrm,prm,sclvec,arrvec
  ...
  sclname=['radar.revision.major','radar.revision.minor', $
           ...
           'txpl', 'mpinc','mppul','mplgs','mplgs','nrang','frang', $
           'rsep','xcf','tfreq', 'mxpwr','lvmax','combf']

  scltype=[1,1,1,9,9,2,2,2,2,2,2,2,2,3,2,2,2,2,2,2,2,2,4,4,2,2,4,2,2,2,2,3, $
           2,2,2,2,2,2,2,2,2,2,3,3,9]
  ...
  if (sclid[35] ne -1) then prm.mplgs=*(sclvec[sclid[36]].ptr)
  if (sclid[36] ne -1) then prm.nrang=*(sclvec[sclid[37]].ptr)
  if (sclid[37] ne -1) then prm.frang=*(sclvec[sclid[38]].ptr)
  if (sclid[38] ne -1) then prm.rsep=*(sclvec[sclid[39]].ptr)
  if (sclid[39] ne -1) then prm.xcf=*(sclvec[sclid[40]].ptr)
  if (sclid[40] ne -1) then prm.tfreq=*(sclvec[sclid[41]].ptr)
  if (sclid[41] ne -1) then prm.mxpwr=*(sclvec[sclid[42]].ptr)
  if (sclid[42] ne -1) then prm.lvmax=*(sclvec[sclid[43]].ptr)
