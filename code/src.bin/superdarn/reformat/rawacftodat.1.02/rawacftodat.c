...
int main(int argc,char *argv[]) {
  ...
  if (OldRawHeaderFwrite(stdout,"rawwrite",vstring,thr,            
			"example file") !=0) {
  ...
  do {
    ...
    if (rprm.nrang>ORIG_MAX_RANGE) {
      ...
      OldRawFwrite(stdout,"rawwrite",&rprm,&rawacf,recnum,rtab);  
    } else  OldRawFwrite(stdout,"rawwrite",&rprm,&rawacf,recnum,NULL);
