...
/*
 $Log: oldgrdidl.c,v $
 Revision 1.2  2008/03/25 16:20:07  code
 Fixed pointer sign.

 Revision 1.1  2004/11/02 22:27:23  barnes
 Initial revision

*/

int32 OldGridIDLPoint(int argc,char *argv[]) {
  int fildes;
  int unit;
  IDL_FILE_STAT stat;
  FILE *fp;
...
int32 OldGridIDLRead(int argc,char *argv[]) {
...
int32 OldGridIDLDecode(int argc,char *argv[]) {
  int n;
  int yr,mo,dy,hr,mt;
  double sc;
  char *ptr;
...
int32 OldGridIDLWrite(int argc,char *argv[]) {
...
int32 OldGridIDLSeek(int argc,char *argv[]) {
  ...
  if (inx.num !=0) s=OldGridFseek(fp,yr,mo,dy,hr,mt,(int) sc,&inx,atme);
  else s=OldGridFseek(fp,yr,mo,dy,hr,mt,(int) sc,NULL,atme);
