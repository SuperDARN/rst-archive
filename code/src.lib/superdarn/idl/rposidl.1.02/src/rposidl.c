...
int32 RPosGeochrtIDL(int argc,char *argv[]) {
  ...
  for ( rncrd=0; rncrd<75; rncrd++) {
    for ( bmcrd=0; bmcrd<16; bmcrd++) {
      ...
      rho[ 16* rncrd + bmcrd]= rho_int;
      lat[ 16* rncrd + bmcrd]= lat_int;
      lng[ 16* rncrd + bmcrd]= lng_int;
      azm[ 16* rncrd + bmcrd]= azm_int;
      elv[ 16* rncrd + bmcrd]= elv_int;
     }
  }
