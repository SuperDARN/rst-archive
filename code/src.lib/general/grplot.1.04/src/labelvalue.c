...
int GrPlotLabelValueStart(char *name,int atnum,char **atname,char **atval,
  ...
  struct GrPlotLabelValue *ptr;
  struct GrPlotData *plt;

  blk=(struct GrPlotBlock *) data;
  if (blk==NULL) return -1;
  ptr=(struct GrPlotLabelValue *) blk->data;
 ...
 if (strcmp(name,"limit")==0) {
  ...
  if (strcmp(name,"value")==0) {
    ...
    blk=GrPlotSave(blk->xml,"value",val);
...
int GrPlotLabelValueEnd(char *name,char *buf,int sze,void *data) {
  ...
  if (strcmp(name,blk->name)==0) {
    ...
    if (strcmp(name,"xlabelvalue")==0)
      PlotXaxisLabelValue(plot,plt->num,ptr->min,ptr->max,ptr->val,
                     ptr->flg,strlen(ptr->txt),ptr->txt,
                     ptr->fname,ptr->fsize,ptr->color,ptr->mask);

    if (strcmp(name,"ylabelvalue")==0)
      PlotYaxisLabelValue(plot,plt->num,ptr->min,ptr->max,ptr->val,
                          ptr->flg,strlen(ptr->txt),ptr->txt,
                          ptr->fname,ptr->fsize,ptr->color,ptr->mask);
