...
int SiteCveSetupRadar() {
  ...
  if (debug) {
    fprintf(stderr,"GET_SITE_SETTINGS:ifmode=%d\n",ifmode);
  }
...
int SiteCveIntegrate(int (*lags)[2]) {
  ...
  printf("Size of Struct DataPRM  %ld\n",sizeof(struct DataPRM));
    ...
    if(dprm.status==0) {
      if (debug) {
        fprintf(stderr,"CVE GET_DATA: rdata.main: uint32: %ld array: %ld\n",sizeof(uint32),sizeof(uint32)*dprm.samples);
      }
      ...
      if (debug) 
        fprintf(stderr,"CVE GET_DATA: badtrdat.start_usec: uint32: %ld array: %ld\n",sizeof(uint32),sizeof(uint32)*badtrdat.length);
...
*/
   ...
   SiteCveExit(0);
   if (debug) {
     fprintf(stderr,"CVE SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",iqsze,iqsze/2,iqsze/4);
