...
int SiteTstSetupRadar() {
  ...
  sprintf(sharedmemory,"IQBuff_Tst_%d_%d",rnum,cnum);
...
int SiteTstIntegrate(int (*lags)[2]) {
  ...
  while (1) {
    ...
    if(dprm.status==0) {
      ...
      if (debug) {
        fprintf(stderr,"GET_DATA: trtimes length\n");
      }
...
/* AJ End edit */
    ...
    /* calculate ACF */   
      ...
      fprintf(stderr,"SiteTstIntegrate:: Bad pulse sequence status: %d\n",dprm.status);
      fflush(stderr);
    }
    gettimeofday(&tick,NULL);
